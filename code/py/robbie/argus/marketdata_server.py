...
if __name__ == '__main__':
    ...
    taskenv.setObj('env', 'application', 'marketdata' )
    ...
    if 'run_tag' in tweaks:
        tweaks[ 'run_tradeDate' ]   = tweaks[ 'run_tag' ]
    else:
    ...
    with twkcx.Tweaks( **tweaks ):
        ...
        if twisted:
            ...
            import meadow.argus.servershell_util as svrshutil
            server = svrshutil.Server( cmputil.taskData )
            cmputil.initComponents()
    
            firstExec   = 'always'
            tag         = 'PRESOD'
    
            dailytask.setTask( tag='PRESOD', taskName='resetserver', args={ 'server': server } )
            dailytask.scheduleDailyTasks( logger=logger, tag=tag, firstExec=firstExec )
